...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            # claude code
            # 统计valid/invalid数量
            num_invalid = response_invalid_mask.sum().item()
            num_valid = response_invalid_mask.numel() - num_invalid
            invalid_rate = num_invalid / response_invalid_mask.numel()
            print(f"[Rank {self.rank}] Valid/Invalid: {num_valid} valid, {num_invalid} invalid ({invalid_rate*100:.2f}%)")
            #
