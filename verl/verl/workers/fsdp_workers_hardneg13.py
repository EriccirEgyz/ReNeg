# 利用query和positive document合成hard negative。
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    # claude code
    def __del__(self):
        if hasattr(self, 'reward_stats_file') and self.reward_stats_file is not None:
            self.reward_stats_file.close()
    #
    ...
    # claude code
    def _log_reward_stats(self, scores: torch.Tensor, step: int = None, raw_queries=None):
        """Log reward distribution statistics (per-query by default)

        Args:
            scores: reward scores tensor, shape (batch_size,)
            step: current training step
            raw_queries: list of raw query strings for grouping
        """
        scores_cpu = scores.detach().cpu().float()

        step_str = f"step={step}" if step is not None else f"count={self.reward_stats_count}"

        if raw_queries is not None and len(raw_queries) == len(scores_cpu):
            # Group by actual query content
            query_groups = {}
            for i, (query, score) in enumerate(zip(raw_queries, scores_cpu)):
                #query_key = query[:50] + "..." if len(query) > 50 else query
                query_key = query
                if query_key not in query_groups:
                    query_groups[query_key] = []
                query_groups[query_key].append(score.item())

            if self.rank == 0:
                print(f"\n[RewardModelWorker] {step_str} - Per-query reward statistics:")
                print(f"Total samples: {len(scores_cpu)}, Unique queries: {len(query_groups)}")
                for query_key, scores_list in query_groups.items():
                    scores_tensor = torch.tensor(scores_list)
                    mean = scores_tensor.mean().item()
                    std = scores_tensor.std().item()
                    min_val = scores_tensor.min().item()
                    max_val = scores_tensor.max().item()
                    p25 = scores_tensor.quantile(0.25).item()
                    p50 = scores_tensor.quantile(0.50).item()
                    p75 = scores_tensor.quantile(0.75).item()

                    print(f"  Query: \"{query_key}\"")
                    print(f"    Responses: {len(scores_list)} | "
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            pos_outputs = self.reward_module(
                input_ids=micro_batch.batch["pos_input_ids"],
                attention_mask=micro_batch.batch["pos_attention_mask"],
                use_cache=False
            )
            ...
            pos_embeds = self._last_token_pool(pos_outputs.last_hidden_state, micro_batch.batch['pos_attention_mask'])
            ...
            neg_sim = (query_embeds * hardneg_embeds).sum(dim=1)
            # *逐元素相乘，query_embeds * hardneg_embeds结果是 [batch_size, hidden_dim]
            pos_sim = (query_embeds * pos_embeds).sum(dim=1)
            ...
            url = "http://10.144.202.36:30000" # server ip需要调整！！！
            ...
            # claude code
            # 准备所有mask（确保都是相同device的tensor）
            format_invalid_mask = micro_batch.batch["response_invalid_flags"].bool()
            judge_invalid_mask = torch.from_numpy(judge_scores).to(neg_sim.device) == 0
            # 计算query和hardneg的长度（字符数）
            query_lengths = torch.tensor([len(q) for q in raw_queries], device=neg_sim.device)
            hardneg_lengths = torch.tensor([len(h) for h in raw_hardnegs], device=neg_sim.device)
            length_valid_mask = (hardneg_lengths >= (query_lengths * 3)) # 这里有超参数需要调整

            # 超参数定义
            k_threshold = 0.8  # k的饱和阈值

            # 计算k = neg_sim / pos_sim (添加epsilon防止除零)
            k = neg_sim / (pos_sim + 1e-8)

            # 基础reward: min(k/0.8, 1.0)
            base_reward = torch.minimum(k / k_threshold, torch.tensor(1.0, device=k.device))

            # 条件mask（1表示满足条件）
            format_valid_mask = ~format_invalid_mask
            judge_valid_mask = ~judge_invalid_mask
            
            # 最终条件乘数（所有条件都必须满足）
            condition_multiplier = format_valid_mask.float() * judge_valid_mask.float() * length_valid_mask.float()

            # 计算最终reward
            rm_scores = base_reward * condition_multiplier

            # 统计各类invalid
            num_format_invalid = format_invalid_mask.sum().item()
            num_judge_invalid = judge_invalid_mask.sum().item()
            num_length_invalid = (~length_valid_mask).sum().item()
            num_valid = (condition_multiplier > 0).sum().item()

            print(f"<reward breakdown> format_invalid: {num_format_invalid}; "
                  f"judge_invalid: {num_judge_invalid}; "
                  f"length_invalid: {num_length_invalid}; "
                  f"valid: {num_valid}; "
                  f"total: {len(neg_sim)}")
    ...
    def _get_anchor_sample(self, data: DataProto): # 在compute_rm_score中用到了
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            pos_enc = target_tokenizer(pos, padding='max_length', truncation=True, max_length=max_length, return_tensors="pt")
            pos_input_ids.append(pos_enc["input_ids"])
            pos_attention_mask.append(pos_enc["attention_mask"])
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # claude code
            # Log reward statistics (after all micro batches processed)
            # Gather scores and queries from all GPUs for complete statistics
            if torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
                world_size = torch.distributed.get_world_size()

                # Gather scores from all GPUs
                gathered_scores = [torch.zeros_like(scores) for _ in range(world_size)]
                torch.distributed.all_gather(gathered_scores, scores)
                all_scores = torch.cat(gathered_scores, dim=0)

                # Gather queries from all GPUs using gather_object (for Python objects)
                local_queries = rm_data.non_tensor_batch["raw_queries"] 
                # 如果micro_batches经过重排，这里不一定是真实顺序，建议从micro_batches中收集queries
                #local_queries = rm_data.non_tensor_batch.get("raw_queries", [])

                if self.rank == 0:
                    # Only rank 0 needs to provide the gather list
                    gathered_queries = [None] * world_size
                    torch.distributed.gather_object(local_queries, gathered_queries, dst=0)
                    # Flatten the list of lists
                    raw_queries = []
                    for qlist in gathered_queries:
                        if qlist is not None:
                            raw_queries.extend(qlist)
                else:
                    # Non-rank-0 processes pass None as gather_list
                    torch.distributed.gather_object(local_queries, None, dst=0)
                    raw_queries = None

            else:
                all_scores = scores
                raw_queries = rm_data.non_tensor_batch["raw_queries"]
                #raw_queries = rm_data.non_tensor_batch.get("raw_queries", None)

            # Only rank 0 logs to avoid duplicate output in distributed training
            if self.rank == 0:
                self._log_reward_stats(
                    all_scores,
                    step=data.meta_info.get("global_step", None) if hasattr(data, "meta_info") else None,
