...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            doc_outputs = self.reward_module(
                input_ids=micro_batch.batch["doc_input_ids"],
                attention_mask=micro_batch.batch["doc_attention_mask"],
                use_cache=False
            )
            query_embeds = self._last_token_pool(query_outputs.last_hidden_state, micro_batch.batch['query_attention_mask'])
            doc_embeds = self._last_token_pool(doc_outputs.last_hidden_state, micro_batch.batch['doc_attention_mask'])
            query_embeds = F.normalize(query_embeds, p=2, dim=1)
            doc_embeds = F.normalize(doc_embeds, p=2, dim=1)

            similarities = (query_embeds * doc_embeds).sum(dim=1)
    ...
    def _get_anchor_sample(self, data: DataProto): # 在compute_rm_score中用到了
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            # 1. extract document
            doc = data.non_tensor_batch["reward_model"][i]["document"]
            raw_docs.append(doc)
            if self.rank == 0 and i == 0:
                # for debugging purpose
                print(f"document for reward embedding model: {doc}")

            doc_enc = target_tokenizer(doc, padding='max_length', truncation=True, max_length=max_length, return_tensors="pt")
            doc_input_ids.append(doc_enc["input_ids"])
            doc_attention_mask.append(doc_enc["attention_mask"])

            # 2. extract the generated query
            ...
            if match:
                try:
                    ...
                    if "query" in generation_json:
                        query = generation_json["query"]
                        invalid_flag = 0
            ...
            if self.rank == 0 and i == 0:
                # for debugging purpose
                print("提取response json中的query字段内容作为生成的query")
                ...
                print("如果没按照要求json格式生成，或者在目前的max_response_length下没有生成完全，rewards直接设为0。")
                print("----------")
        ...
        doc_input_ids = torch.cat(doc_input_ids, dim=0)
        doc_attention_mask = torch.cat(doc_attention_mask, dim=0)
        response_invalid_flags = torch.cat(response_invalid_flags, dim=0)

        # 选择只返回一个DataProto，方便后续切分成micro batch
        tensors = {"query_input_ids": query_input_ids, "query_attention_mask": query_attention_mask, 
            "doc_input_ids": doc_input_ids, "doc_attention_mask": doc_attention_mask,
            "response_invalid_flags": response_invalid_flags}
        non_tensors = {"raw_docs": raw_docs, "raw_queries": raw_queries}
