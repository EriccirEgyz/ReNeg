...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            url = "http://10.144.207.113:30000" # server ip需要调整！！！
            ...
            length_valid_mask = (hardneg_lengths >= 200) # 这里有超参数需要调整!

            # 最终valid mask（所有条件都必须满足）
            ...
            print(f"<reward breakdown> format_invalid: {num_format_invalid}; "
                  ...
                  f"valid (reward=1): {num_valid}; "
                  f"total: {len(neg_sim)}")

            # 设置二值化reward（默认为0，只有final_valid为1）
