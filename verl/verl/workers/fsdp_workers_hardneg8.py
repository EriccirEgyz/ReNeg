...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            # claude code
            # === 明确的优先级链设计 ===
            # Priority 1: format_invalid (JSON解析失败) → reward=0
            # Priority 2: judge_invalid (judge认为不是valid hardneg) → reward=0
            # Priority 3: similarity_invalid (neg_sim < pos_sim * 0.8) → reward=0
            # 只有三个条件都满足 → reward=1

            # 1. 准备所有mask（确保都是相同device的tensor）
            ...
            sim_valid_mask = (neg_sim >= (pos_sim * 0.7)) # 这里有超参数需要调整!

            # 2. 最终valid mask（三个条件都满足）
            final_valid_mask = sim_valid_mask & ~format_invalid_mask & ~judge_invalid_mask
            ...
            # 4. 设置二值化reward（默认为0，只有final_valid为1）
