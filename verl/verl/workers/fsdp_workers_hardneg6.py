# 利用query和positive document合成hard negative。用sim(query, hardneg)做稀疏化reward
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            scores = torch.cat(output, dim=0)  # (batch_size) - 本地scores，例如64个
            # claude code
            local_batch_size = scores.shape[0]  # 保存本地batch大小
            #

            # claude code
            # === Gather scores and queries from all GPUs ===
            if torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
                ...
                if self.rank == 0:
                    ...
                    raw_queries_gathered = []
                    for qlist in gathered_queries:
                        if qlist is not None:
                            raw_queries_gathered.extend(qlist)
                else:
                    # Non-rank-0 processes pass None as gather_list
                    torch.distributed.gather_object(local_queries, None, dst=0)
                    raw_queries_gathered = None

            else:
                print("没有检测到分布式训练")
                all_scores = scores
                raw_queries_gathered = rm_data.non_tensor_batch["raw_queries"]
            # ===================================================================

            # === Log reward statistics (only on rank 0) ===
            if self.rank == 0:
                self._log_reward_stats(
                    all_scores, # 稀疏化前的结果
                    step=data.meta_info.get("global_step", None) if hasattr(data, "meta_info") else None,
                    raw_queries=raw_queries_gathered
                )
            # ===================================================================

            # === Sparse reward computation: per-query max-based binarization ===
            # For each query, find max reward a, then binarize:
            #   reward > a - 0.05 -> 1
            #   reward <= a - 0.05 -> 0
            # Perform on rank 0 after gathering, then broadcast to all ranks

            if torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
                if self.rank == 0:
                    # Create query-to-indices mapping
                    query_to_indices = {}
                    for idx, query in enumerate(raw_queries_gathered):
                        if query not in query_to_indices:
                            query_to_indices[query] = []
                        query_to_indices[query].append(idx)

                    # Create sparse scores tensor (binarized: 0 or 1)
                    sparse_scores = torch.zeros_like(all_scores)
                    ...
                    for query, indices in query_to_indices.items():
                        ...
                        query_scores = all_scores[indices]
                        max_score = query_scores.max().item()

                        # Binarize: 1 if score > max - 0.05, else 0
                        query_mask = (query_scores > (max_score - 0.05)).float()
                        sparse_scores[indices] = query_mask

                    all_scores = sparse_scores

                # Broadcast sparse scores from rank 0 to all other ranks
                torch.distributed.broadcast(all_scores, src=0)
            else:
                # Single GPU case: compute sparse rewards directly
                print("没有检测到分布式训练")
                query_to_indices = {}
                for idx, query in enumerate(raw_queries_gathered):
                    if query not in query_to_indices:
            ...
            # 从全局all_scores中提取本地对应的scores
            # all_scores: e.g. 全局512个，每个GPU需要提取自己对应的本地部分（64个）
            if torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
                global_offset = local_batch_size * self.rank  # 0, 64, 128, 196, ...
                scores = all_scores[global_offset : global_offset + local_batch_size]
            else:
                scores = all_scores
            #
