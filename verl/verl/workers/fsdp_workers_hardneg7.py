...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            url = "http://10.158.151.84:30001" # server ip需要调整！！！
            ...
            # 开始长度检测，字符数小于length_threshold，reward就为0
            length_threshold = 200
            hardneg_char_lens = torch.tensor(
                [len(hn) for hn in raw_hardnegs],
                dtype=rm_scores.dtype,
                device=rm_scores.device,
            )
            num_short = (hardneg_char_lens < length_threshold).sum().item()
            print(f"<length statistics> too short : {num_short}; total num: {len(hardneg_char_lens)}")
            length_invalid_mask = hardneg_char_lens < length_threshold
            rm_scores[length_invalid_mask] = 0
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            # For each query, normalize scores to [0, 1] range:
            ...
            if torch.distributed.is_initialized() and torch.distributed.get_world_size() > 1:
                if self.rank == 0:
                    ...
                    # Create normalized scores tensor
                    normalized_scores = torch.zeros_like(all_scores)
                    ...
                    for query, indices in query_to_indices.items():
                        ...
                        min_score = query_scores.min().item()
                        max_score = query_scores.max().item()

                        # Min-max normalization to [0, 1]
                        if max_score - min_score > 1e-8:  # 避免除零
                            query_normalized = (query_scores - min_score) / (max_score - min_score)
                        else:
                            # 如果所有分数相同，映射到1
                            query_normalized = torch.ones_like(query_scores)

                        normalized_scores[indices] = query_normalized

                    all_scores = normalized_scores

                # Broadcast normalized scores from rank 0 to all other ranks
                torch.distributed.broadcast(all_scores, src=0)
            else:
                # Single GPU case: compute normalized rewards directly
                print("没有检测到分布式训练")
                query_to_indices = {}
                for idx, query in enumerate(raw_queries_gathered):
                    if query not in query_to_indices:
