...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            url = "http://10.144.201.14:30000" # server ip需要调整！！！
            ...
            length_valid_mask = (hardneg_lengths >= (query_lengths * 2)) # 这里有超参数需要调整

            # 最终valid mask（所有条件都必须满足）：生成了json格式 + 被judge为true negative + 长度满足要求
            final_valid_mask = (~format_invalid_mask &
                                ~judge_invalid_mask &
                                length_valid_mask)
            ...
            # 计算reward：sim(query, hardneg) * 1[生成了指定的json格式] * 1[被judge为true negative] * 1[str长度大于等于query的2倍]
            rm_scores = neg_sim.clone()
            rm_scores[~final_valid_mask] = 0  # 不满足条件时reward为0
